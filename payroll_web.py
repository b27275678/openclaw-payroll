...
function showResults(){
  ...
  payrollData.forEach(p=>{
    ...
    html+=`<div class="stub">
      <h3>${p.name} — Check Date: ${new Date().toLocaleDateString()}</h3>
      ...
      <div class="row total"><span>NET CHECK</span><span>$${p.net.toFixed(2)}</span></div>
    </div>`;
  });
